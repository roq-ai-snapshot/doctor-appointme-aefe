...
function AcademyListPage() {
  ...
  const { data, error, isLoading, mutate } = useSWR<AcademyInterface[]>(
    ...
    () =>
      getAcademies({
        relations: ['user_academy_user_idTouser', 'appointment.count', 'user_user_academy_idToacademy.count'],
      }),
  );
              ...
              <Tbody>
                {data?.map((record) => (
                  <Tr cursor="pointer" onClick={() => handleView(record.id)} key={record.id}>
                    ...
                    {hasAccess('user', AccessOperationEnum.READ, AccessServiceEnum.PROJECT) && (
                      <Td>
                        <Link as={NextLink} href={`/users/view/${record.user_academy_user_idTouser?.id}`}>
                          {record.user_academy_user_idTouser?.email}
                        </Link>
                      </Td>
                    )}
                    {hasAccess('appointment', AccessOperationEnum.READ, AccessServiceEnum.PROJECT) && (
                    ...
                    {hasAccess('user', AccessOperationEnum.READ, AccessServiceEnum.PROJECT) && (
                      <Td>{record?._count?.user_user_academy_idToacademy}</Td>
                    )}
                    <Td>

...
function AcademyViewPage() {
  ...
  const { data, error, isLoading, mutate } = useSWR<AcademyInterface>(
    ...
    () =>
      getAcademyById(id, {
        relations: ['user_academy_user_idTouser', 'appointment'],
      }),
  );
            ...
            <Box>
              {hasAccess('user', AccessOperationEnum.READ, AccessServiceEnum.PROJECT) && (
                <Flex alignItems="center" mb={4}>
                  <Text fontSize="lg" fontWeight="bold" as="span">
                    User Academy User Id Touser:
                  </Text>
                  <Text fontSize="md" as="span" ml={3}>
                    <Link as={NextLink} href={`/users/view/${data?.user_academy_user_idTouser?.id}`}>
                      {data?.user_academy_user_idTouser?.email}
                    </Link>
                  </Text>
                </Flex>
              )}
              {hasAccess('appointment', AccessOperationEnum.READ, AccessServiceEnum.PROJECT) && (
                <Stack spacing={2} mb={8}>
                  <Flex justifyContent="space-between">
                    ...
                    <NextLink passHref href={`/appointments/create?academy_id=${data?.id}`}>

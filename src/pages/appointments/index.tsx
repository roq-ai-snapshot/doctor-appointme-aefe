...
function AppointmentListPage() {
  ...
  const { data, error, isLoading, mutate } = useSWR<AppointmentInterface[]>(
    ...
    () =>
      getAppointments({
        relations: [
          'user_appointment_coach_idTouser',
          'user_appointment_player_idTouser',
          'user_appointment_parent_idTouser',
          'academy',
        ],
      }),
  );
              ...
              <Thead>
                <Tr>
                  ...
                  {hasAccess('user', AccessOperationEnum.READ, AccessServiceEnum.PROJECT) && (
                    <Th>user_appointment_coach_idTouser</Th>
                  )}
                  {hasAccess('user', AccessOperationEnum.READ, AccessServiceEnum.PROJECT) && (
                    <Th>user_appointment_player_idTouser</Th>
                  )}
                  {hasAccess('user', AccessOperationEnum.READ, AccessServiceEnum.PROJECT) && (
                    <Th>user_appointment_parent_idTouser</Th>
                  )}
              ...
              <Tbody>
                {data?.map((record) => (
                  <Tr cursor="pointer" onClick={() => handleView(record.id)} key={record.id}>
                    ...
                    {hasAccess('user', AccessOperationEnum.READ, AccessServiceEnum.PROJECT) && (
                      <Td>
                        <Link as={NextLink} href={`/users/view/${record.user_appointment_coach_idTouser?.id}`}>
                          {record.user_appointment_coach_idTouser?.email}
                        </Link>
                      </Td>
                    )}
                    {hasAccess('user', AccessOperationEnum.READ, AccessServiceEnum.PROJECT) && (
                      <Td>
                        <Link as={NextLink} href={`/users/view/${record.user_appointment_player_idTouser?.id}`}>
                          {record.user_appointment_player_idTouser?.email}
                        </Link>
                      </Td>
                    )}
                    {hasAccess('user', AccessOperationEnum.READ, AccessServiceEnum.PROJECT) && (
                      <Td>
                        <Link as={NextLink} href={`/users/view/${record.user_appointment_parent_idTouser?.id}`}>
                          {record.user_appointment_parent_idTouser?.email}
                        </Link>
                      </Td>
                    )}
